...
!CDIR NOVERRCHK
      ...
      ! derive co2(aq) and pCO2 for use in other bio routines
               zfact2 = rhop(ji,jj,jk) / 1000. + rtrn
               zdic  = trb(ji,jj,jk,jpdic)
               zph   = MAX( hi(ji,jj,jk), 1.e-10 ) / zfact2
            ! CALCULATE [H2CO3]
            co2aq(ji,jj,jk) = zdic/(1. + ak13(ji,jj,jk)/zph + ak13(ji,jj,jk)*ak23(ji,jj,jk)/zph**2)
            END DO
            pco2s(ji,jj) = co2aq(ji,jj,1)/chemc(ji,jj,1)
         END DO
      END DO      
