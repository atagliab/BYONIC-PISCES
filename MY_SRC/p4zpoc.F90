...
CONTAINS
   ...
   SUBROUTINE p4z_poc( kt, knt )
      ...
      REAL(wp) ::   zrfact2
      REAL(wp) ::   zocub, zocub2, zocub3 !Cam copper model
      REAL(wp) ::   zocob, zocob2, zocob3
      REAL(wp) ::   zomnb, zomnb2, zomnb3
      REAL(wp) ::   zozin, zozin2, zozin3
      REAL(wp) ::   zdust
      CHARACTER (len=25) :: charout
      REAL(wp), DIMENSION(jpi,jpj  )   :: totprod, totthick, totcons 
      REAL(wp), DIMENSION(jpi,jpj,jpk)   :: zremipoc, zremigoc, zorem3, ztremint, zfolimi, zculimi, zzolimi, zcolimi, zmnlimi
     ...
     DO jk = 2, jpkm1
      ...
      ENDIF
                 ...
                 IF ( ln_copper ) THEN
                 zocub2 = zremig * trb(ji,jj,jk,jpcug)
                 zocub3 = zremig * solgoc * trb(ji,jj,jk,jpcug)
                 tra(ji,jj,jk,jpdcu) = tra(ji,jj,jk,jpdcu) + zocub2
                 tra(ji,jj,jk,jpcug) = tra(ji,jj,jk,jpcug) - zocub2 - zocub3
                 tra(ji,jj,jk,jpcup) = tra(ji,jj,jk,jpcup) + zocub3
                 zculimi(ji,jj,jk) = zocub2
                 ENDIF
                 IF ( ln_zinc ) THEN
                 zozin2 = zremig * trb(ji,jj,jk,jpzng)
                 zozin3 = zremig * solgoc * trb(ji,jj,jk,jpzng)
                 tra(ji,jj,jk,jpdzn) = tra(ji,jj,jk,jpdzn) + zozin2
                 tra(ji,jj,jk,jpzng) = tra(ji,jj,jk,jpzng) - zozin2 - zozin3
                 tra(ji,jj,jk,jpznp) = tra(ji,jj,jk,jpznp) + zofer3
                 zzolimi(ji,jj,jk) = zozin2
                 ENDIF
                 IF ( ln_cobalt ) THEN
                 zocob2 = zremig * trb(ji,jj,jk,jpcog)
                 zocob3 = zremig * solgoc * trb(ji,jj,jk,jpcog)
                 tra(ji,jj,jk,jpdco) = tra(ji,jj,jk,jpdco) + zocob2
                 tra(ji,jj,jk,jpcog) = tra(ji,jj,jk,jpcog) - zocob2 - zocob3
                 tra(ji,jj,jk,jpcop) = tra(ji,jj,jk,jpcop) + zocob3
                 zcolimi(ji,jj,jk) = zocob2
                 ENDIF
                 IF ( ln_manganese ) THEN
                 zomnb2 = zremig * trb(ji,jj,jk,jpmng)
                 zomnb3 = zremig * solgoc * trb(ji,jj,jk,jpmng)
                 tra(ji,jj,jk,jpdmn) = tra(ji,jj,jk,jpdmn) + zomnb2
                 tra(ji,jj,jk,jpmng) = tra(ji,jj,jk,jpmng) - zomnb2 - zomnb3
                 tra(ji,jj,jk,jpmnp) = tra(ji,jj,jk,jpmnp) + zomnb3
                 zmnlimi(ji,jj,jk) = zomnb2
                 ENDIF
     ...
     IF( ln_p4z ) THEN
      ...
      ENDIF
                    ...
                    IF (ln_copper) THEN
                 zocub           = zremip * trb(ji,jj,jk,jpcup)
                 tra(ji,jj,jk,jpdcu) = tra(ji,jj,jk,jpdcu) + zocub
                 tra(ji,jj,jk,jpcup) = tra(ji,jj,jk,jpcup) - zocub
                 zculimi(ji,jj,jk)   = zculimi(ji,jj,jk) + zocub
                  ENDIF
                    IF (ln_zinc) THEN
                 zozin           = zremip * trb(ji,jj,jk,jpznp)
                 tra(ji,jj,jk,jpdzn) = tra(ji,jj,jk,jpdzn) + zozin
                 tra(ji,jj,jk,jpznp) = tra(ji,jj,jk,jpznp) - zozin
                 zzolimi(ji,jj,jk)   = zzolimi(ji,jj,jk) + zozin
                  ENDIF
                    IF (ln_cobalt) THEN
                 zocob           = zremip * trb(ji,jj,jk,jpcop)
                 tra(ji,jj,jk,jpdco) = tra(ji,jj,jk,jpdco) + zocob
                 tra(ji,jj,jk,jpcop) = tra(ji,jj,jk,jpcop) - zocob
                 zcolimi(ji,jj,jk)   = zcolimi(ji,jj,jk) + zocob
                  ENDIF
                    IF (ln_manganese) THEN
                 zomnb           = zremip * trb(ji,jj,jk,jpmnp)
                 tra(ji,jj,jk,jpdmn) = tra(ji,jj,jk,jpdmn) + zomnb
                 tra(ji,jj,jk,jpmnp) = tra(ji,jj,jk,jpmnp) - zomnb
                 zmnlimi(ji,jj,jk)   = zmnlimi(ji,jj,jk) + zomnb
                  ENDIF
     ...
     IF( lk_iomput ) THEN
        ...
        IF ( ln_copper ) THEN
          CALL iom_put( "REMINCU" , zculimi(:,:,:) * tmask(:,:,:)  * 1.e+9 * zrfact2 )  ! Remineralisation rate
        ENDIF
        IF ( ln_cobalt ) THEN
          CALL iom_put( "REMINC" , zcolimi(:,:,:) * tmask(:,:,:)  * 1.e+9 * zrfact2 )  ! Remineralisation rate
        ENDIF
        IF ( ln_manganese ) THEN
          CALL iom_put( "REMINMN" , zmnlimi(:,:,:) * tmask(:,:,:)  * 1.e+9 * zrfact2 )  ! Remineralisation rate
        ENDIF

        IF ( ln_zinc ) THEN
          CALL iom_put( "REMINZ" , zzolimi(:,:,:) * tmask(:,:,:)  * 1.e+9 * zrfact2 )  ! Remineralisation rate
        ENDIF
   ...
   SUBROUTINE p4z_poc_init
      ...
      !!
      NAMELIST/nampispoc/ xremip , jcpoc  , rshape,  ln_dustrfe, kdust, &
