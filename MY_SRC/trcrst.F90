...
CONTAINS
   ...
   SUBROUTINE trc_rst_read
      ...
      END DO
      !
!      trb(:,:,:,jpdmn) = max(0.,4E-9, tsn(:,:,:,jp_tem)/5E9) ; trn(:,:,:,jpdmn) = trb(:,:,:,jpdmn)
!      trb(:,:,:,jpdmn) = max(0.4E-9, 5E-9 * EXP( -0.005 * gdept_n(:,:,:) ) ) ; trn(:,:,:,jpdmn) = trb(:,:,:,jpdmn)
!      trn(:,:,:,jpmnd) = trn(:,:,:,jpdia) * 1.E-6 ; trb(:,:,:,jpmnd) = trn(:,:,:,jpmnd)
!      trn(:,:,:,jpmnn) = trn(:,:,:,jpphy) * 1.E-6 ; trb(:,:,:,jpmnn) = trn(:,:,:,jpmnn)
!      trn(:,:,:,jpmng) = trn(:,:,:,jpgoc) * 0.1E-6 ; trb(:,:,:,jpmng) = trn(:,:,:,jpmng)
!      trn(:,:,:,jpmnp) = trn(:,:,:,jppoc) * 0.1E-6 ; trb(:,:,:,jpmnp) = trn(:,:,:,jpmnp)
