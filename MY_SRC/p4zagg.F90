...
CONTAINS
   ...
   SUBROUTINE p4z_agg ( kt, knt )
      ...
      REAL(wp) ::   zaggcu !Cam
      REAL(wp) ::   zaggzn !Zn model
      REAL(wp) ::   zaggco
      REAL(wp) ::   zaggmn ! Mn model
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  ...
                  !
     IF( ln_zinc ) THEN
               zaggzn = zagg * trb(ji,jj,jk,jpznp) / ( trb(ji,jj,jk,jppoc) + rtrn )
               tra(ji,jj,jk,jpznp) = tra(ji,jj,jk,jpznp) - zaggzn
               tra(ji,jj,jk,jpzng) = tra(ji,jj,jk,jpzng) + zaggzn
     ENDIF
     IF( ln_cobalt ) THEN
               zaggco = zagg * trb(ji,jj,jk,jpcop) / ( trb(ji,jj,jk,jppoc) + rtrn )
               tra(ji,jj,jk,jpcop) = tra(ji,jj,jk,jpcop) - zaggco
               tra(ji,jj,jk,jpcog) = tra(ji,jj,jk,jpcog) + zaggco
     ENDIF
     IF( ln_manganese ) THEN
               zaggmn = zagg * trb(ji,jj,jk,jpmnp) / ( trb(ji,jj,jk,jppoc) + rtrn )
               tra(ji,jj,jk,jpmnp) = tra(ji,jj,jk,jpmnp) - zaggmn
               tra(ji,jj,jk,jpmng) = tra(ji,jj,jk,jpmng) + zaggmn
     ENDIF

     IF( ln_copper ) THEN
               zaggcu = zagg * trb(ji,jj,jk,jpcup) / ( trb(ji,jj,jk,jppoc) + rtrn )
               tra(ji,jj,jk,jpcup) = tra(ji,jj,jk,jpcup) - zaggcu
               tra(ji,jj,jk,jpcug) = tra(ji,jj,jk,jpcug) + zaggcu
     ENDIF
               END DO
            END DO
         END DO
