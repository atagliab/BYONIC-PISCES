...
CONTAINS
   ...
   SUBROUTINE p4z_nano
      ...
      REAL(wp) ::   zfactcu, zfactzn, zfactco, zfactmn                              ! Cam Cu model
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               if (ln_copper ) THEN
               zfactcu = trb(ji,jj,jk,jpcun)/( trb(ji,jj,jk,jpphy) + rtrn )
               tra(ji,jj,jk,jpcun) = tra(ji,jj,jk,jpcun) - zmortp * zfactcu
               tra(ji,jj,jk,jpcup) = tra(ji,jj,jk,jpcup) + ( 1. - zfracal ) * zmortp * zfactcu
               tra(ji,jj,jk,jpcug) = tra(ji,jj,jk,jpcug) + zfracal * zmortp * zfactcu
               ENDIF
               if (ln_zinc ) THEN
               zfactzn = trb(ji,jj,jk,jpznn)/( trb(ji,jj,jk,jpphy) + rtrn )
               tra(ji,jj,jk,jpznn) = tra(ji,jj,jk,jpznn) - zmortp * zfactzn
               tra(ji,jj,jk,jpznp) = tra(ji,jj,jk,jpznp) + ( 1. - zfracal ) * zmortp * zfactzn
               tra(ji,jj,jk,jpzng) = tra(ji,jj,jk,jpzng) + zfracal * zmortp * zfactzn
               ENDIF
               if (ln_cobalt ) THEN
               zfactco = trb(ji,jj,jk,jpcon)/( trb(ji,jj,jk,jpphy) + rtrn )
               tra(ji,jj,jk,jpcon) = tra(ji,jj,jk,jpcon) - zmortp * zfactco
               tra(ji,jj,jk,jpcop) = tra(ji,jj,jk,jpcop) + ( 1. - zfracal ) * zmortp * zfactco
               tra(ji,jj,jk,jpcog) = tra(ji,jj,jk,jpcog) + zfracal * zmortp * zfactco
               ENDIF
               if (ln_manganese ) THEN
               zfactmn = trb(ji,jj,jk,jpmnn)/( trb(ji,jj,jk,jpphy) + rtrn )
               tra(ji,jj,jk,jpmnn) = tra(ji,jj,jk,jpmnn) - zmortp * zfactmn
               tra(ji,jj,jk,jpmnp) = tra(ji,jj,jk,jpmnp) + ( 1. - zfracal ) * zmortp * zfactmn
               tra(ji,jj,jk,jpmng) = tra(ji,jj,jk,jpmng) + zfracal * zmortp * zfactmn
               ENDIF
   ...
   SUBROUTINE p4z_diat
      ...
      REAL(wp) ::   zrespp2, ztortp2, zmortp2, zfactcu, zfactzn, zfactco, zfactmn
      REAL(wp) ::   zlim2, zlim1
      CHARACTER (len=25) ::   charout
      !!---------------------------------------------------------------------
      !
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               IF (ln_copper) THEN
               zfactcu = trb(ji,jj,jk,jpcud) / ( trb(ji,jj,jk,jpdia) + rtrn )
               tra(ji,jj,jk,jpcud) = tra(ji,jj,jk,jpcud) - zmortp2 * zfactcu
               tra(ji,jj,jk,jpcup) = tra(ji,jj,jk,jpcup) + 0.5 * ztortp2 * zfactcu
               tra(ji,jj,jk,jpcug) = tra(ji,jj,jk,jpcug) + ( zrespp2 + 0.5 * ztortp2 ) * zfactcu
               ENDIF
               IF (ln_zinc) THEN
               zfactzn = trb(ji,jj,jk,jpznd) / ( trb(ji,jj,jk,jpdia) + rtrn )
               tra(ji,jj,jk,jpznd) = tra(ji,jj,jk,jpznd) - zmortp2 * zfactzn
               tra(ji,jj,jk,jpzfd) = tra(ji,jj,jk,jpzfd) - zmortp2 * trb(ji,jj,jk,jpzfd) &
               &  / ( trb(ji,jj,jk,jpdia) + rtrn )
               tra(ji,jj,jk,jpznf) = tra(ji,jj,jk,jpznf) + zmortp2 * trb(ji,jj,jk,jpzfd) &
               &  / ( trb(ji,jj,jk,jpdia) + rtrn )
               tra(ji,jj,jk,jpznp) = tra(ji,jj,jk,jpznp) + 0.5 * ztortp2 * zfactzn
               tra(ji,jj,jk,jpzng) = tra(ji,jj,jk,jpzng) + ( zrespp2 + 0.5 * ztortp2 ) * zfactzn
               ENDIF
                IF (ln_cobalt) THEN
               zfactco = trb(ji,jj,jk,jpcod) / ( trb(ji,jj,jk,jpdia) + rtrn )
               tra(ji,jj,jk,jpcod) = tra(ji,jj,jk,jpcod) - zmortp2 * zfactco
               tra(ji,jj,jk,jpcop) = tra(ji,jj,jk,jpcop) + 0.5 * ztortp2 * zfactco
               tra(ji,jj,jk,jpcog) = tra(ji,jj,jk,jpcog) + ( zrespp2 + 0.5 * ztortp2 ) * zfactco
               ENDIF
                IF (ln_manganese) THEN
               zfactmn = trb(ji,jj,jk,jpmnd) / ( trb(ji,jj,jk,jpdia) + rtrn )
               tra(ji,jj,jk,jpmnd) = tra(ji,jj,jk,jpmnd) - zmortp2 * zfactmn
               tra(ji,jj,jk,jpmnp) = tra(ji,jj,jk,jpmnp) + 0.5 * ztortp2 * zfactmn
               tra(ji,jj,jk,jpmng) = tra(ji,jj,jk,jpmng) + ( zrespp2 + 0.5 * ztortp2 ) * zfactmn
               ENDIF
