MODULE p4zsms
   ...
   REAL(wp) ::   oxybudget, copperbudget, zincbudget, cobaltbudget, manganesebudget
...
CONTAINS
   ...
   SUBROUTINE p4z_sms_init
      ...
      NAMELIST/nampisbio/ nrdttrc, wsbio, xkmort, ferat3, wsbio2, wsbio2max, wsbio2scale,    &
         &                   ldocp, ldocz, lthet, no3rat3, po4rat3, curat3, znrat3, znf, ln_znf, corat3, mnrat3
         !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nampisbio in configuration namelist', lwp )
      ...
      IF(lwp) THEN                         ! control print
         ...
         ENDIF
         IF ( ln_copper ) THEN
         WRITE(numout,*) '    Cu/C in zooplankton                       curat3 =', curat3
          ENDIF
         IF ( ln_zinc ) THEN
         WRITE(numout,*) '    Zn/C in zooplankton                       znrat3 =', znrat3
         WRITE(numout,*) '    Proportion of diatom Zinc in the frustule     znf =', znf
         WRITE(numout,*) '    Fe affects proportion of diatom Zinc in the frustule ln_ znf =', ln_znf
          ENDIF
         IF ( ln_cobalt ) THEN
         WRITE(numout,*) '    Co/C in zooplankton                       corat3 =', corat3
          ENDIF
         IF ( ln_manganese ) THEN
         WRITE(numout,*) '    Mn/C in zooplankton                       mnrat3 =', mnrat3
          ENDIF
...
908   IF( ios >  0 )   CALL ctl_nam ( ios , 'nampismass in configuration namelist', lwp )
   ...
   SUBROUTINE p4z_chk_mass( kt )
      ...
      ENDIF
      IF( iom_use( "pcoppertot" ) .OR. ( ln_check_mass .AND. kt == nitend )  ) THEN
         zwork(:,:,:) =   trn(:,:,:,jpdcu) + trn(:,:,:,jpcun) + trn(:,:,:,jpcud) &
            &         +   trn(:,:,:,jpcup) + trn(:,:,:,jpcug) + trn(:,:,:,jpscup) + trn(:,:,:,jpscug) &
            &         + ( trn(:,:,:,jpzoo) + trn(:,:,:,jpmes) )  * curat3
         !
         copperbudget = glob_sum( 'p4zsms', zwork(:,:,:) * cvol(:,:,:)  )
         copperbudget = copperbudget / areatot
         CALL iom_put( "pcoppertot", copperbudget )
      ENDIF
      IF( iom_use( "pzinctot" ) .OR. ( ln_check_mass .AND. kt == nitend )  ) THEN
         zwork(:,:,:) =   trn(:,:,:,jpdzn) + trn(:,:,:,jpznn) + trn(:,:,:,jpznd) + trn(:,:,:,jpzfd) +  trn(:,:,:,jpznf)   &
            &         +   trn(:,:,:,jpznp) + trn(:,:,:,jpzng) + trn(:,:,:,jpszp) + trn(:,:,:,jpszg) &
            &         + ( trn(:,:,:,jpzoo) + trn(:,:,:,jpmes) )  * znrat3
         !
         zincbudget = glob_sum( 'p4zsms', zwork(:,:,:) * cvol(:,:,:)  )
         zincbudget = zincbudget / areatot
         CALL iom_put( "pzinctot", zincbudget )
      ENDIF
      IF( iom_use( "pcobalttot" ) .OR. ( ln_check_mass .AND. kt == nitend )  ) THEN
         zwork(:,:,:) =   trn(:,:,:,jpdco) + trn(:,:,:,jpcon) + trn(:,:,:,jpcod) &
            &         +   trn(:,:,:,jpcop) + trn(:,:,:,jpcog)  + trn(:,:,:,jpsco) &
            &         + ( trn(:,:,:,jpzoo) + trn(:,:,:,jpmes) )  * corat3
         !
         cobaltbudget = glob_sum( 'p4zsms', zwork(:,:,:) * cvol(:,:,:)  )
         cobaltbudget = cobaltbudget / areatot
         CALL iom_put( "pcobalttot", cobaltbudget )
      ENDIF
      IF( iom_use( "pmanganesetot" ) .OR. ( ln_check_mass .AND. kt == nitend )  ) THEN
         zwork(:,:,:) =   trn(:,:,:,jpdmn) + trn(:,:,:,jpmnn) + trn(:,:,:,jpmnd) &
            &         +   trn(:,:,:,jpmnp) + trn(:,:,:,jpmng)  + trn(:,:,:,jpsmn) &
            &         + ( trn(:,:,:,jpzoo) + trn(:,:,:,jpmes) )  * mnrat3
         !
         manganesebudget = glob_sum( 'p4zsms', zwork(:,:,:) * cvol(:,:,:)  )
         manganesebudget = manganesebudget / areatot
         CALL iom_put( "pmanganesetot", manganesebudget )
      ENDIF
