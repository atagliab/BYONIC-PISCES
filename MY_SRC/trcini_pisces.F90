...
CONTAINS
   ...
   SUBROUTINE p4z_ini
      ...
      DO jn = 1, jptra
        ...
        IF( cltra == 'DCU'      )   jpdcu = jn      !:
        IF( cltra == 'CUN'      )   jpcun = jn      !:
        IF( cltra == 'CUD'      )   jpcud = jn      !:
        IF( cltra == 'CUP'      )   jpcup = jn      !:
        IF( cltra == 'CUG'      )   jpcug = jn      !:
        IF( cltra == 'SCUP'     )   jpscup = jn      !:
        IF( cltra == 'SCUG'     )   jpscug = jn      !:
        IF( cltra == 'DZN'      )   jpdzn = jn      !:
        IF( cltra == 'ZNN'      )   jpznn = jn      !:
        IF( cltra == 'ZND'      )   jpznd = jn      !:
        IF( cltra == 'ZNP'      )   jpznp = jn      !:
        IF( cltra == 'ZNG'      )   jpzng = jn      !:
        IF( cltra == 'ZNF'      )   jpznf = jn      !:
        IF( cltra == 'ZFD'      )   jpzfd = jn      !:
        IF( cltra == 'SZP'      )   jpszp = jn      !:
        IF( cltra == 'SZG'      )   jpszg = jn      !:
        IF( cltra == 'DCO'      )   jpdco = jn      !:
        IF( cltra == 'CON'      )   jpcon = jn      !:
        IF( cltra == 'COD'      )   jpcod = jn      !:
        IF( cltra == 'COP'      )   jpcop = jn      !:
        IF( cltra == 'COG'      )   jpcog = jn      !:
        IF( cltra == 'SCO'      )   jpsco = jn      !:
        IF( cltra == 'DMN'      )   jpdmn = jn      !:
        IF( cltra == 'MNN'      )   jpmnn = jn      !:
        IF( cltra == 'MND'      )   jpmnd = jn      !:
        IF( cltra == 'MNP'      )   jpmnp = jn      !:
        IF( cltra == 'MNG'      )   jpmng = jn      !:
        IF( cltra == 'SMN'      )   jpsmn = jn      !:
      END DO
      ...
      ENDIF
       IF( ln_copper ) THEN
         zcu2c     = 2._wp / 122._wp  ! molCu mol P-1 (Ho)
       ENDIF
       IF( ln_zinc ) THEN
        zzn2c     = 1.E-3  / 122._wp   ! 1 mmol Zn mol P-1 (Ho)
        zzn2s     = 0.065 / 1000._wp   ! 0.065 nmol Zn / umol Si (Lohan)
       ENDIF
       IF( ln_cobalt ) THEN
         zco2c     = 100.E-6 / 122._wp  ! 0.1 mmol Co mol P-1 (Ho)
       ENDIF
       IF( ln_manganese ) THEN
         zmn2c     = 0.36E-3 / 122._wp  ! 0.36 mmol Mn mol P-1 (Ho)
       ENDIF
      ...
      IF( .NOT.ln_rsttr ) THEN  
         ...
         IF( ln_copper ) THEN
         trn(:,:,:,jpdcu) = 2.5E-9
         trn(:,:,:,jpcun) = bioma0 * zcu2c
         trn(:,:,:,jpcud) = bioma0 * zcu2c
         trn(:,:,:,jpcup) = bioma0 * zcu2c
         trn(:,:,:,jpcug) = bioma0 * zcu2c
         trn(:,:,:,jpscup) = bioma0 * zcu2c
         trn(:,:,:,jpscug) = bioma0 * zcu2c
         ENDIF
         IF( ln_zinc ) THEN
         trn(:,:,:,jpdzn) = 5E-9
         trn(:,:,:,jpznn) = bioma0 * zzn2c
         trn(:,:,:,jpznd) = bioma0 * zzn2c
         trn(:,:,:,jpznp) = bioma0 * zzn2c
         trn(:,:,:,jpzng) = bioma0 * zzn2c
         trn(:,:,:,jpznf) = bioma0 * zzn2c * 0.03
         trn(:,:,:,jpzfd) = bioma0 * zzn2c * 0.03
         trn(:,:,:,jpszp) = bioma0 * zzn2c / 1000.
         trn(:,:,:,jpszg) = bioma0 * zzn2c / 1000.
         ENDIF
         IF( ln_cobalt ) THEN
         trn(:,:,:,jpdco) = 50E-12 * 1000.
         trn(:,:,:,jpcon) = bioma0 * zco2c * 1000.
         trn(:,:,:,jpcod) = bioma0 * zco2c * 1000.
         trn(:,:,:,jpcop) = bioma0 * zco2c * 1000.
         trn(:,:,:,jpcog) = bioma0 * zco2c * 1000.
         trn(:,:,:,jpsco) = bioma0 * zco2c * 1000.
         ENDIF
         IF( ln_manganese ) THEN
         trn(:,:,:,jpdmn) = 3E-9
         trn(:,:,:,jpmnn) = bioma0 * zmn2c
         trn(:,:,:,jpmnd) = bioma0 * zmn2c
         trn(:,:,:,jpmnp) = bioma0 * zmn2c
         trn(:,:,:,jpmng) = bioma0 * zmn2c
         trn(:,:,:,jpsmn) = bioma0 * zmn2c
         ENDIF
