...
CONTAINS
   ...
   SUBROUTINE p4z_meso( kt, knt )
      ...
      REAL(wp) :: zepsherq, zepsherf, zepshert, zepsherv, zgrarsig, zgraztotc, zgraztotn, zgraztotf
...
!Cam copper model
      REAL(wp) :: zepsherqcu, zepshercu, zepshertcu, zgrasratcu, zgrazfffgcu
      REAL(wp) :: zgraznfcu, zgrazfcu, zgrazpofcu, zgrazfffpcu, zgracu2, zgraztotfcu, zfraccu
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zcuzoo2   
! Cam Zn Model
      REAL(wp) :: zepsherz, zepsherqz, zepshertz, zgrasratz, zgrazfffgz
      REAL(wp) :: zgraznfz, zgrazfz, zgrazpofz, zgrazfffpz, zgrazn2, zgraztotfz, zfraczn
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zznzoo2
! Cam co model
      REAL(wp) :: zepsherc, zepshertc, zepsherqc,  zgrasratc, zgrazfffgc
      REAL(wp) :: zgraznfc, zgrazfc, zgrazpofc, zgrazfffpc, zgraco2, zgraztotfc, zfracco
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zcozoo2
! Cam Mn model
      REAL(wp) :: zepsherm, zepshertm, zgrasratm, zepsherqm, zgrazfffgm
      REAL(wp) :: zgraznfm, zgrazfm, zgrazpofm, zgrazfffpm, zgramn2, zgraztotfm, zfracmn
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zmnzoo2
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               IF (ln_zinc) THEN
               zgraznfz   = zgrazn   * trb(ji,jj,jk,jpznn) / ( trb(ji,jj,jk,jpphy) + rtrn)
               zgrazfz    = zgrazd   * trb(ji,jj,jk,jpznd) / ( trb(ji,jj,jk,jpdia) + rtrn)
               zgrazpofz  = zgrazpoc * trb(ji,jj,jk,jpznp) / ( trb(ji,jj,jk,jppoc) + rtrn)
               ENDIF
 
               IF (ln_copper) THEN
               zgraznfcu   = zgrazn   * trb(ji,jj,jk,jpcun) / ( trb(ji,jj,jk,jpphy) + rtrn)
               zgrazfcu    = zgrazd   * trb(ji,jj,jk,jpcud) / ( trb(ji,jj,jk,jpdia) + rtrn)
               zgrazpofcu  = zgrazpoc * trb(ji,jj,jk,jpcup) / ( trb(ji,jj,jk,jppoc) + rtrn)
               ENDIF
               IF (ln_cobalt) THEN
               zgraznfc   = zgrazn   * trb(ji,jj,jk,jpcon) / ( trb(ji,jj,jk,jpphy) + rtrn)
               zgrazfc    = zgrazd   * trb(ji,jj,jk,jpcod) / ( trb(ji,jj,jk,jpdia) + rtrn)
               zgrazpofc  = zgrazpoc * trb(ji,jj,jk,jpcop) / ( trb(ji,jj,jk,jppoc) + rtrn)
               ENDIF
               IF (ln_manganese) THEN
               zgraznfm   = zgrazn   * trb(ji,jj,jk,jpmnn) / ( trb(ji,jj,jk,jpphy) + rtrn)
               zgrazfm    = zgrazd   * trb(ji,jj,jk,jpmnd) / ( trb(ji,jj,jk,jpdia) + rtrn)
               zgrazpofm  = zgrazpoc * trb(ji,jj,jk,jpmnp) / ( trb(ji,jj,jk,jppoc) + rtrn)
               ENDIF
               ...
               !
              IF ( ln_copper ) zgrazfffpcu = zgrazffep * trb(ji,jj,jk,jpcup) / (trb(ji,jj,jk,jppoc) + rtrn)
              IF ( ln_copper ) zgrazfffgcu = zgrazffeg * trb(ji,jj,jk,jpcug) / (trb(ji,jj,jk,jpgoc) + rtrn)

              IF ( ln_zinc ) zgrazfffpz = zgrazffep * trb(ji,jj,jk,jpznp) / (trb(ji,jj,jk,jppoc) + rtrn)
              IF ( ln_zinc ) zgrazfffgz = zgrazffeg * trb(ji,jj,jk,jpzng) / (trb(ji,jj,jk,jpgoc) + rtrn)
              IF ( ln_cobalt ) zgrazfffpc = zgrazffep * trb(ji,jj,jk,jpcop) / (trb(ji,jj,jk,jppoc) + rtrn)
              IF ( ln_cobalt ) zgrazfffgc = zgrazffeg * trb(ji,jj,jk,jpcog) / (trb(ji,jj,jk,jpgoc) + rtrn)
              IF ( ln_manganese ) zgrazfffpm = zgrazffep * trb(ji,jj,jk,jpmnp) / (trb(ji,jj,jk,jppoc) + rtrn)
              IF ( ln_manganese ) zgrazfffgm = zgrazffeg * trb(ji,jj,jk,jpmng) / (trb(ji,jj,jk,jpgoc) + rtrn)
              ...
              IF ( ln_copper ) THEN
              zfraccu   = zfrac * trb(ji,jj,jk,jpcug) / (trb(ji,jj,jk,jpgoc) + rtrn)
              zgrazfffpcu = zproport * zgrazfffpcu
              zgrazfffgcu = zproport * zgrazfffgcu
              zgraztotfcu = zgrazfcu + zgraznfcu + zgrazz * ( curat3 ) + zgrazpofcu  &
              &            + zgrazfffpcu + zgrazfffgcu
              ENDIF
              IF ( ln_zinc ) THEN
              zfraczn   = zfrac * trb(ji,jj,jk,jpzng) / (trb(ji,jj,jk,jpgoc) + rtrn)
              zgrazfffpz = zproport * zgrazfffpz
              zgrazfffgz = zproport * zgrazfffgz
              zgraztotfz = zgrazfz + zgraznfz + zgrazz * znrat3 + zgrazpofz  &
              &            + zgrazfffpz + zgrazfffgz
              ENDIF
              IF ( ln_cobalt ) THEN
              zfracco   = zfrac * trb(ji,jj,jk,jpcog) / (trb(ji,jj,jk,jpgoc) + rtrn)
              zgrazfffpc = zproport * zgrazfffpc
              zgrazfffgc = zproport * zgrazfffgc
              zgraztotfc = zgrazfc + zgraznfc + zgrazz * ( corat3 * 1000. ) + zgrazpofc  &
              &            + zgrazfffpc + zgrazfffgc
              ENDIF
              IF ( ln_manganese ) THEN
              zfracmn   = zfrac * trb(ji,jj,jk,jpmng) / (trb(ji,jj,jk,jpgoc) + rtrn)
              zgrazfffpm = zproport * zgrazfffpm
              zgrazfffgm = zproport * zgrazfffgm
              zgraztotfm = zgrazfm + zgraznfm + zgrazz * ( mnrat3 ) + zgrazpofm  &
              &            + zgrazfffpm + zgrazfffgm
              ENDIF
...
!              zepsherv  = zepshert * MIN( zepsherf, (1. - unass2) * zgrasrat /ferat3, (1. - unass2) * zgrasratn ) 
              ...
              IF ( ln_copper ) THEN
              zgrasratcu  = ( zgraztotfcu + rtrn ) / ( zgraztotc + rtrn )
...
!              zepshercu  = zepshertcu * MIN( epsher2, (1. - unass2) * zgrasratcu / ( curat3 ) , (1. - unass2) * zgrasratn )
               zgracu2  = zgraztotc * MAX( 0. , ( 1. - unass2 ) * zgrasratcu - (curat3 ) * zepshercu )    &
                &       + ( curat3 ) * ( ( 1. - epsher2 - unass2 ) /( 1. - epsher2 ) * ztortz )
               ENDIF
               IF ( ln_zinc ) THEN
...
!              zepsherz  = zepshertz * MIN( epsher2, (1. - unass2) * zgrasratz / znrat3, (1. - unass2) * zgrasratn )
               zgrazn2  = zgraztotc * MAX( 0. , ( 1. - unass2 ) * zgrasratz - znrat3 * zepsherz )    &
                &       + znrat3 * ( ( 1. - epsher2 - unass2 ) /( 1. - epsher2 ) * ztortz )
               ENDIF
              IF ( ln_cobalt ) THEN
...
!              zepsherc  = zepshertc * MIN( epsher2, (1. - unass2) * zgrasratc / ( corat3 * 1000.) , (1. - unass2) * zgrasratn )
               zgraco2  = zgraztotc * MAX( 0. , ( 1. - unass2 ) * zgrasratc - (corat3 * 1000. ) * zepsherc )    &
                &       + ( corat3 * 1000. ) * ( ( 1. - epsher2 - unass2 ) /( 1. - epsher2 ) * ztortz )
               ENDIF
              IF ( ln_manganese ) THEN
...
!              zepsherm  = zepshertm * MIN( epsher2, (1. - unass2) * zgrasratm / ( mnrat3 ) , (1. - unass2) * zgrasratn )
              zgramn2  = zgraztotc * MAX( 0. , ( 1. - unass2 ) * zgrasratm - (mnrat3 ) * zepsherm )    &
                &       + ( mnrat3 ) * ( ( 1. - epsher2 - unass2 ) /( 1. - epsher2 ) * ztortz )
               ENDIF
               ...
               IF ( ln_copper ) THEN
               tra(ji,jj,jk,jpdcu) = tra(ji,jj,jk,jpdcu) + zgracu2
               zcuzoo2(ji,jj,jk) = zgracu2

               tra(ji,jj,jk,jpcun) = tra(ji,jj,jk,jpcun) - zgraznfcu
               tra(ji,jj,jk,jpcud) = tra(ji,jj,jk,jpcud) - zgrazfcu
               tra(ji,jj,jk,jpcup) = tra(ji,jj,jk,jpcup) + ( curat3 ) * zmortzgoc - zgrazfffpcu - zgrazpofcu    &
                 &                 + zgraztotfcu * unass2
               tra(ji,jj,jk,jpcup) = tra(ji,jj,jk,jpcup) - zgrazpofcu - zgrazfffpcu + zfraccu
               tra(ji,jj,jk,jpcug) = tra(ji,jj,jk,jpcug) + ( curat3 ) * zmortzgoc -zgrazfffgcu &
               &                     + zgraztotfcu * unass2 - zfraccu
               ENDIF
               IF ( ln_cobalt ) THEN
               tra(ji,jj,jk,jpdco) = tra(ji,jj,jk,jpdco) + zgraco2
               zcozoo2(ji,jj,jk) = zgraco2
               tra(ji,jj,jk,jpcon) = tra(ji,jj,jk,jpcon) - zgraznfc
               tra(ji,jj,jk,jpcod) = tra(ji,jj,jk,jpcod) - zgrazfc
               tra(ji,jj,jk,jpcop) = tra(ji,jj,jk,jpcop) + ( corat3 * 1000. ) * zmortzgoc - zgrazfffpc - zgrazpofc    &
                 &                 + zgraztotfc * unass2
               tra(ji,jj,jk,jpcop) = tra(ji,jj,jk,jpcop) - zgrazpofc - zgrazfffpc + zfracco
               tra(ji,jj,jk,jpcog) = tra(ji,jj,jk,jpcog) + ( corat3 * 1000. ) * zmortzgoc -zgrazfffgc &
               &                     + zgraztotfc * unass2 - zfracco
               ENDIF
               IF ( ln_manganese ) THEN
               tra(ji,jj,jk,jpdmn) = tra(ji,jj,jk,jpdmn) + zgramn2
               zmnzoo2(ji,jj,jk) = zgramn2
               tra(ji,jj,jk,jpmnn) = tra(ji,jj,jk,jpmnn) - zgraznfm
               tra(ji,jj,jk,jpmnd) = tra(ji,jj,jk,jpmnd) - zgrazfm
               tra(ji,jj,jk,jpmnp) = tra(ji,jj,jk,jpmnp) + ( mnrat3 ) * zmortzgoc - zgrazfffpm - zgrazpofm    &
                 &                 + zgraztotfm * unass2
               tra(ji,jj,jk,jpmnp) = tra(ji,jj,jk,jpmnp) - zgrazpofm - zgrazfffpm + zfracmn
               tra(ji,jj,jk,jpmng) = tra(ji,jj,jk,jpmng) + ( mnrat3 ) * zmortzgoc -zgrazfffgm &
               &                     + zgraztotfm * unass2 - zfracmn
               ENDIF

               IF ( ln_zinc ) THEN
               tra(ji,jj,jk,jpdzn) = tra(ji,jj,jk,jpdzn) + zgrazn2
               zznzoo2(ji,jj,jk) = zgrazn2
               tra(ji,jj,jk,jpzfd) = tra(ji,jj,jk,jpzfd) - zgrazfz *trb(ji,jj,jk,jpzfd) / ( trb(ji,jj,jk,jpznd) + rtrn )
               tra(ji,jj,jk,jpznf) = tra(ji,jj,jk,jpznf) + zgrazfz * trb(ji,jj,jk,jpzfd) / ( trb(ji,jj,jk,jpznd) + rtrn )
               tra(ji,jj,jk,jpznn) = tra(ji,jj,jk,jpznn) - zgraznfz
               tra(ji,jj,jk,jpznd) = tra(ji,jj,jk,jpznd) - zgrazfz
               tra(ji,jj,jk,jpznp) = tra(ji,jj,jk,jpznp) + znrat3 * zmortzgoc - zgrazfffpz - zgrazpofz    &
                 &                 + zgraztotfz * unass2
               tra(ji,jj,jk,jpznp) = tra(ji,jj,jk,jpznp) - zgrazpofz - zgrazfffpz + zfraczn
               tra(ji,jj,jk,jpzng) = tra(ji,jj,jk,jpzng) + znrat3 * zmortzgoc - zgrazfffgz &
               &                     + zgraztotfz * unass2 - zfraczn
               ENDIF
      ...
      IF( lk_iomput .AND. knt == nrdttrc ) THEN
         ...
         ENDIF
IF (ln_copper) THEN 
         IF( iom_use( "CUZOO2" ) ) THEN
            zw3d(:,:,:) = zcuzoo2(:,:,:) * 1e9 * 1.e+3 * rfact2r * tmask(:,:,:)
            CALL iom_put( "CUZOO2", zw3d )
         ENDIF
ENDIF
IF (ln_zinc) THEN 
         IF( iom_use( "ZNZOO2" ) ) THEN
            zw3d(:,:,:) = zznzoo2(:,:,:) * 1e9 * 1.e+3 * rfact2r * tmask(:,:,:)
            CALL iom_put( "ZNZOO2", zw3d )
         ENDIF
ENDIF
IF (ln_cobalt) THEN 
         IF( iom_use( "COZOO2" ) ) THEN
            zw3d(:,:,:) = zcozoo2(:,:,:) * 1e9 * 1.e+3 * rfact2r * tmask(:,:,:)
            CALL iom_put( "COZOO2", zw3d )
         ENDIF
ENDIF
IF (ln_manganese) THEN
         IF( iom_use( "MNZOO2" ) ) THEN
            zw3d(:,:,:) = zmnzoo2(:,:,:) * 1e9 * 1.e+3 * rfact2r * tmask(:,:,:)
            CALL iom_put( "MNZOO2", zw3d )
         ENDIF
ENDIF
