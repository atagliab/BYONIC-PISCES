MODULE p4zsbc
   ...
   REAL(wp), PUBLIC :: hydrocu
   REAL(wp), PUBLIC :: cusolub
   REAL(wp), PUBLIC :: cufrac      !: Cam frqction of cu in dust
   LOGICAL , PUBLIC :: ln_cudust   !: cam boolean for inclusion of Cu from dust or not
   LOGICAL , PUBLIC :: ln_curiv    !: cam boolean for Cu from rivers
   REAL(wp), PUBLIC  :: hydrozn
   REAL(wp), PUBLIC  :: znsolub
   REAL(wp), PUBLIC  :: znrivconc
   REAL(wp), PUBLIC  :: susfr
   REAL(wp), PUBLIC  :: cosolub
   REAL(wp), PUBLIC  :: co_o2_1
   REAL(wp), PUBLIC  :: co_o2_2
   REAL(wp), PUBLIC  :: sedco
   LOGICAL , PUBLIC  :: ln_codust
   LOGICAL , PUBLIC  :: ln_cosed
   LOGICAL , PUBLIC  :: ln_coriv
   REAL(wp), PUBLIC  :: corivconc
   REAL(wp), PUBLIC :: hydromn
   REAL(wp), PUBLIC :: mnsolub
   REAL(wp), PUBLIC :: mnfrac      !: Cam frqction of cu in dust
   LOGICAL , PUBLIC :: ln_mndust   !: cam boolean for inclusion of Cu from dust or not
   LOGICAL , PUBLIC :: ln_mnriv    !: cam boolean for Cu from rivers
   REAL(wp), PUBLIC  :: mn_o2_1
   REAL(wp), PUBLIC  :: sedmn
   LOGICAL , PUBLIC  :: ln_mnsed
   REAL(wp), PUBLIC  :: mnrivconc
   ...
   TYPE(FLD), ALLOCATABLE, DIMENSION(:) ::   sf_cudust      ! structure of inputcudust
   ...
   INTEGER             ::   ntimes_solub, ntimes_hydro, ntimes_cudust             ! number of time steps in a file

   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   dust  , cudust, solub    !: dust fields
...
CONTAINS
   ...
   SUBROUTINE p4z_sbc( kt )
      ...
      ENDIF
      !
      ! Compute cu dust at nit000 or only if there is more than 1 time record in
      ! cu dust file
      IF( ln_cudust ) THEN
         IF( kt == nit000 .OR. ( kt /= nit000 .AND. ntimes_cudust > 1 ) ) THEN
            CALL fld_read( kt, 1, sf_cudust )
            cudust(:,:) = MAX( rtrn, sf_cudust(1)%fnow(:,:,1) ) * ( 1.0 - fr_i(:,:) )
         ENDIF
      ENDIF
   ...
   SUBROUTINE p4z_sbc_init
      ...
      INTEGER  :: numdust, numsolub, numriv, numiron, numdepo, numhydro, numcudust
      ...
      REAL(wp) :: ztimes_dust, ztimes_riv, ztimes_ndep , ztimes_cudust
      REAL(wp), DIMENSION(:), ALLOCATABLE :: rivinput
      REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: zriver, zcmask, zcudust
      !
      CHARACTER(len=100) ::  cn_dir          ! Root directory for location of ssr files
      TYPE(FLD_N), DIMENSION(jpriv) ::  slf_river    ! array of namelist informations on the fields to read
      TYPE(FLD_N) ::   sn_dust, sn_solub, sn_ndepo, sn_ironsed, sn_hydrofe, sn_cudust   ! informations about the fields to be read
      ...
      !!
      NAMELIST/nampissbc/cn_dir, sn_dust, sn_cudust, sn_solub, sn_riverdic, sn_riverdoc, sn_riverdin, sn_riverdon,     &
        &                sn_riverdip, sn_riverdop, sn_riverdsi, sn_ndepo, sn_ironsed, sn_hydrofe, &
        &                ln_dust, ln_solub, ln_river, ln_ndepo, ln_ironsed, ln_ironice, ln_hydrofe, ln_cudust, ln_curiv,   &
        &                sedfeinput, distcoast, dustsolub, icefeinput, wdust, mfrac, nitrfix, diazolight, concfediaz, &
        &                hratio, lgw_rath, hydrocu, cufrac, cusolub, hydrozn, znsolub, susfr, znrivconc,       &
        &                ln_codust, ln_cosed, ln_coriv, corivconc,cosolub, co_o2_1, co_o2_2, sedco,         &
        &                hydromn, mnfrac, mnsolub, ln_mndust, ln_mnsed, ln_mnriv, mnrivconc,mnsolub, mn_o2_1, sedmn 
      !!----------------------------------------------------------------------
      !
      IF(lwp) THEN
         WRITE(numout,*)
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nampissbc in configuration namelist', lwp )
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '    Copper solubility in dust                cusolub       =', cusolub
         WRITE(numout,*) '    copper frqction in dust                cufrac          =', cufrac
         WRITE(numout,*) '   Copper hydrothermal concentration       hyfdrocu        =', hydrocu
         WRITE(numout,*) '    Inclusion of Cu dust input            ln_cudust        = ', ln_cudust
         WRITE(numout,*) '   Copper from rivers                      ln_curiv        = ', ln_curiv
         WRITE(numout,*) '    Zn/Fe ratio from sed sources                           = ', hydrozn
         WRITE(numout,*) '    Zinc solubility in dust                        znsolub = ', znsolub
         WRITE(numout,*) '    Zinc  concentration in rivers                znrivconc = ', znrivconc
         WRITE(numout,*) ' Fraction of scavenged zinc that is resuspended      susfr = ', susfr
         WRITE(numout,*) '    Cobalt solubility in dust                      cosolub = ', cosolub
         WRITE(numout,*) '    Cobalt concentration in rivers               corivconc = ', corivconc
         WRITE(numout,*) '    Manganese solubility in dust                   mnsolub =', mnsolub
         WRITE(numout,*) '    Manganese fraction in dust                      mnfrac =', mnfrac
         WRITE(numout,*) '   Manganese hydrothermal Mn/Fe                   hyfdromn =', hydromn
         WRITE(numout,*) '    Inclusion ofMn dust input                     ln_mndust= ', ln_mndust
         WRITE(numout,*) '   Manganese from rivers                          ln_mnriv = ', ln_mnriv
      ...
      ! cu dust input from the atmosphere
      ! ------------------------------
      IF( ln_cudust ) THEN
         !
         IF(lwp) WRITE(numout,*) '    initialize dust input from atmosphere '
         IF(lwp) WRITE(numout,*) '    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ '
         !
         ALLOCATE( cudust(jpi,jpj) )    ! allocation
         !
         ALLOCATE( sf_cudust(1), STAT=ierr )           !* allocate and fill sf_sst (forcing structure) with sn_sst
         IF( ierr > 0 )   CALL ctl_stop( 'STOP', 'p4z_sed_init: unable to allocate sf_cudust structure' )
         !
         CALL fld_fill( sf_cudust, (/ sn_cudust /), cn_dir, 'p4z_sed_init', 'Atmospheric cu dust deposition', 'nampissed' )
                                   ALLOCATE( sf_cudust(1)%fnow(jpi,jpj,1)   )
         IF( sn_cudust%ln_tint )     ALLOCATE( sf_cudust(1)%fdta(jpi,jpj,1,2) )
         !
!         IF( Agrif_Root() ) THEN   !  Only on the master grid
!            ! Get total input dust ; need to compute total atmospheric supply of
!            ! Si in a year
!            CALL iom_open (  TRIM( sn_cudust%clname ) , numcudust )
!            ntimes_cudust = iom_getszuld( numcudust )   ! get number of record in file
!         END IF
...
!            ! Si in a year
            CALL iom_open (  TRIM( sn_cudust%clname ) , numcudust )
             ntimes_cudust = iom_getszuld( numcudust )
!            CALL iom_gettime( numcudust, zsteps, kntime=ntimes_cudust)  ! get number of record in file
            ALLOCATE( zcudust(jpi,jpj,ntimes_cudust) )
            DO jm = 1, ntimes_cudust
               CALL iom_get( numcudust, jpdom_data, TRIM( sn_cudust%clvar ), zcudust(:,:,jm), jm )
            END DO
            CALL iom_close( numcudust )
            ztimes_cudust = 1._wp / REAL(ntimes_cudust, wp)
            DEALLOCATE( zcudust)
         ENDIF
      END IF
