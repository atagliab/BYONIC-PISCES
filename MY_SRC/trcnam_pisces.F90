...
CONTAINS
   ...
   SUBROUTINE trc_nam_pisces
      ...
      !!
      NAMELIST/nampismod/ln_p2z, ln_p4z, ln_p5z, ln_ligand, ln_sediment, ln_copper, ln_zinc, ln_cobalt, ln_manganese
      !!----------------------------------------------------------------------
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nampismod in configuration namelist', lwp )
      ...
      IF(lwp) THEN                  ! control print
         ...
         WRITE(numout,*) '      Flag to use copper model             ln_copper   = ', ln_copper
         WRITE(numout,*) '      Flag to use zinc model               ln_zinc     = ', ln_zinc
         WRITE(numout,*) '      Flag to use cobalt model             ln_cobalt   = ', ln_cobalt
         WRITE(numout,*) '      Flag to use manganese model          ln_manganese = ', ln_manganese
      ...
      IF(lwp) THEN                         ! control print
         ...
         IF( ln_copper )  WRITE(numout,*) ' Copper model on'
         IF( ln_zinc )    WRITE(numout,*) ' Zinc model on'
         IF( ln_cobalt )  WRITE(numout,*) ' cobalt model on'
         IF( ln_manganese )  WRITE(numout,*) ' Manganese model on'
      ENDIF
